...
type Beat = { at: number; clock: string; hour: number; headline: string; significance: number; kind: "beat" | "event" };
...
export default function App() {
  ...
  const loadTimeline = useCallback(async (sid: number) => {
    try {
      setBeats(await api<Beat[]>(`/sessions/${sid}/timeline`));
    } catch { /* timeline is supplementary; other calls surface errors */ }
  }, []);
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(() => { checkNew(sid); loadTimeline(sid); }, 60000);
    return () => { ctrl.abort(); window.clearInterval(poll); };
  }, [sessionId, loadRolling, checkNew, loadSessions, loadTimeline]);
      ...
      {sessionId === null ? (
        ...
        <>
          ...
          <section style={{ marginBottom: 16 }}>
            <h2>Timeline</h2>
            {beats.length === 0 ? (
              <p style={{ color: "#6b7280" }}>No plot points yet — key moments appear here as the stream goes.</p>
            ) : (
              <div style={{ maxHeight: 320, overflowY: "auto", borderLeft: "2px solid #e5e7eb", paddingLeft: 14 }}>
                {beats.map((b, i) => (
                  <div key={i}>
                    {(i === 0 || beats[i - 1].hour !== b.hour) && (
                      <div style={{ fontSize: 12, fontWeight: 700, color: "#6b7280", textTransform: "uppercase",
                                    letterSpacing: 0.5, margin: i === 0 ? "0 0 6px" : "12px 0 6px" }}>
                        Hour {b.hour + 1}
                      </div>
                    )}
                    <div style={{ display: "flex", gap: 10, marginBottom: 6, opacity: b.kind === "event" ? 0.85 : 1 }}>
                      <span style={{ color: "#6b7280", fontVariantNumeric: "tabular-nums", minWidth: 40 }}>{b.clock}</span>
                      <span style={{ fontWeight: b.significance >= 4 ? 600 : 400 }}>{b.headline}</span>
                    </div>
                  </div>
                ))}
              </div>
